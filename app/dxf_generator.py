...
def add_tick(msp, position: tuple[float, float], direction: tuple[float, float], size: float = 6):
    x, y = position
...
def draw_single_hook(
    ...
    add_profile_path(
        msp=msp,
        points=[(inner_patch_start[0], inner_patch_start[1], 0), (inner_patch_end[0], inner_patch_end[1], 0)],
        width=width,
    )
